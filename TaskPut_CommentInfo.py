...
CommentInfoSpider_key = 'TX_Video_CommentInfoSpider_key'
CommentInfoSpider = 'CommentInfoSpider'
cid_vid_coll = MongoDBHelper().get_collection(collection_name='cid_vid', database_name='TX_Video')

def task_put():
    delete_old_logs(CommentInfoSpider, 5)
...
def input_onetask(cid_dict):
    ...
    r.rpush(CommentInfoSpider_key, data_str)

def cron_job():
    sched = BlockingScheduler()
    sched.add_job(task_put, 'cron', hour='15', id='TaskPut_CommentInfo')
    sched.start()
